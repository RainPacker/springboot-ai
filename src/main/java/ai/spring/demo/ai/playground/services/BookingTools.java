...
import ai.spring.demo.ai.playground.data.InventoryDetails;
...
public class BookingTools {
	...
	public  record PrintBookingsRequest(String bookingNumber, String name) {
	}

	public record FindMaterialDetailsRequest(String materialNo) {
	}
	...
	@Bean
	@Description("打印机票预定")
	public Function<PrintBookingsRequest, String> printBookings() {
		return request -> {
			System.out.println("#################"+request.bookingNumber()+"::"+ request.name());
			flightBookingService.printBookings();
			return "<a href='http://dcs.weifu.com.cn/sss.pdf'>查看<a/>";
		};
	}

	@Bean
	@Description("查询物料信息")
	public Function<FindMaterialDetailsRequest, List<InventoryDetails>> findMaterialDetailsRequestStringFunction(){


		return request -> {
			return flightBookingService.findMaterialDetails(request.materialNo());
		};
	}
}
